...
func main() {
	...
	w, err := os.Create(filepath.Join(pkgName, "subfont.go"))
	dieErr(err)
	defer w.Close()
	...
	fmt.Fprintf(wd, "	\"github.com/embeddedgo/display/pix\"\n")
	fmt.Fprintf(wd, "	\"github.com/embeddedgo/display/pix/font/font9\"\n")
	fmt.Fprintf(wd, ")\n")
...
func handleData(wd, ws io.Writer, name string) string {
	...
	*s = strings.TrimSuffix(*s, "."+fontSize)
	...
	case *font9.Fixed:
		orgbpp := 8
		if img, ok := d.Bits.(*pix.AlphaN); ok {
			orgbpp = 1 << img.LogN
		}
		...
		printString(ws, "pix"+name, d.Bits.(*pix.AlphaN).Pix)
	case *font9.Variable:
		orgbpp := 8
		if img, ok := d.Bits.(*pix.AlphaN); ok {
			orgbpp = 1 << img.LogN
...
func optimizeVariable(d *font9.Variable) {
	...
	left:
		for sr.Min.X < sr.Max.X {
			for y := sr.Min.Y; y < sr.Max.Y; y++ {
				c := pix.AlphaNModel(bpp).Convert(src.At(sr.Min.X, y))
	...
	right:
		for sr.Max.X > sr.Min.X {
			for y := sr.Min.Y; y < sr.Max.Y; y++ {
				c := pix.AlphaNModel(bpp).Convert(src.At(sr.Max.X-1, y))
				if c.(color.Alpha).A != 0 {
					break right
				}
			}
...
left:
	for {
		for i := 0; i < d.Num(); i++ {
			...
			for y := sr.Min.Y; y < sr.Max.Y; y++ {
				c := pix.AlphaNModel(bpp).Convert(src.At(x, y))
				if c.(color.Alpha).A != 0 {
					break left
				}
			}
...
func printBits(w io.Writer, name string, img *pix.AlphaN) {
	fmt.Fprintf(w, "	Bits: &pix.ImmAlphaN{\n")
